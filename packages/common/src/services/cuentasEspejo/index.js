...
export async function createCuentasEspejo(token, body) {
    const res = await fetch(`${getApiUrl()}/cuentas-espejo/create`, {
        method: 'POST',
        headers: getCommonHeaders(token),
...
export async function updateCuentasEspejo(token, body) {
    ...
    const {id, ...rest} = body;
    const res = await fetch(`${getApiUrl()}/cuentas-espejo/${id}`, {
        method: 'PUT',
        headers: getCommonHeaders(token),
