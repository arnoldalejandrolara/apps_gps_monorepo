...
import { setPushEndpoint } from '../store/slices/authSlice'
import { useDispatch } from 'react-redux'
import { subscribeToNotificationsRequest } from '../services/NotificationsService'
...
export const useNotifications = () => {
  ...
  useEffect(() => {
    ...
    if (currentPlatform === 'ios-safari' && false) {
      ...
      if (isPWAInstalled() && 'Notification' in window) {
        ...
        console.log('iOS Safari requiere que la app esté instalada como PWA para notificaciones')
      }
    } else {
  ...
  useEffect(() => {
    if ('serviceWorker' in navigator) {
      const handleServiceWorkerMessage = (event) => {
        ...
        } else if (event.data && event.data.type === 'LOCAL_NOTIFICATION_SHOWN') {
          console.log('✅ Notificación local mostrada exitosamente:', event.data.data)
        } else if (event.data && event.data.type === 'NOTIFICATION_ERROR') {
          console.error('❌ Error en notificación:', event.data.error)
        } else if (event.data && event.data.type === 'PUSH_NOTIFICATION_SHOWN') {
          console.log('📱 Notificación push mostrada:', event.data.data)
  ...
  const showLocalNotification = useCallback(async (title, options = {}) => {
    console.log('Intentando mostrar notificación local:', { title, options, platform, permission })
    
    if (platform === 'ios-safari' && permission === 'granted' && false) {
      ...
      if (!isAppActive) {
        console.log('⚠️ App no está activa - las notificaciones locales solo funcionan cuando la app está visible en iOS Safari')
        return null
      }
      ...
      try {
        ...
        if ('serviceWorker' in navigator) {
          console.log('Service Worker disponible, esperando que esté listo...')
          await navigator.serviceWorker.ready
          console.log('Service Worker listo')
          
          // Verificar que hay un controller activo
          if (navigator.serviceWorker.controller) {
            console.log('✅ Controller activo encontrado:', navigator.serviceWorker.controller)
            console.log('Estado del controller:', navigator.serviceWorker.controller.state)
            ...
            console.log('📤 Enviando mensaje al Service Worker:', message)
            ...
            const messagePromise = new Promise((resolve, reject) => {
              ...
              const handleResponse = (event) => {
                if (event.data && event.data.type === 'LOCAL_NOTIFICATION_SHOWN') {
                  clearTimeout(timeout)
                  navigator.serviceWorker.removeEventListener('message', handleResponse)
                  resolve({ type: 'service-worker', title, success: true })
                } else if (event.data && event.data.type === 'NOTIFICATION_ERROR') {
                  clearTimeout(timeout)
                  navigator.serviceWorker.removeEventListener('message', handleResponse)
            ...
            console.log('📤 Mensaje enviado al Service Worker, esperando respuesta...')
            
            const result = await messagePromise
            console.log('✅ Respuesta recibida del Service Worker:', result)
            return result
            
          } else {
            console.log('⚠️ No hay controller activo, registrando service worker...')
            const registration = await navigator.serviceWorker.register('/sw.js')
            await navigator.serviceWorker.ready
            
            if (registration.active) {
              console.log('✅ Service Worker registrado y activo:', registration.active)
              ...
              console.log('📤 Mensaje enviado al Service Worker recién registrado')
        ...
        console.log('🔄 Service Worker no disponible, usando Notification API directamente')
        ...
        console.log('✅ Notificación creada directamente:', notification)
        ...
        try {
          console.log('🔄 Intentando fallback con Notification API...')
          const notification = new Notification(title, {
            icon: '/pwa-192x192.png',
            badge: '/pwa-192x192.png',
      ...
      console.log('❌ No se puede mostrar notificación:', {
        platform,
        permission,
        isSupported: platform === 'ios-safari' && permission === 'granted' && false
      })
      return null
    }
  }, [platform, permission])
  ...
  const validateSubscription = useCallback(async (pushSubscription) => {
  ...
  const renewSubscription = useCallback(async (registration) => {
    if (false) {
  ...
  const subscribeToNotifications = useCallback(async (permissionGranted) => {
    console.log(permissionGranted, "permissionGranted subscribeToNotifications");
    ...
    try {
      ...
      const response = await subscribeToNotificationsRequest(token, JSON.stringify({
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        const renewResponse = await subscribeToNotificationsRequest(token, JSON.stringify({
          subscription: pushSubscription,
          userId: 'USER_ID',
          platform: 'standard',
          type: 'push',
        ...
        if (renewResponse.status === 200 || renewResponse.status === 201) {
          setSubscription(pushSubscription)
          dispatch(setPushEndpoint(pushSubscription))
  ...
  }, [isSupported, validateSubscription, renewSubscription, platform, isConnected])

  // Función para verificar y renovar suscripción periódicamente
  const checkAndRenewSubscription = useCallback(async (serverUrl) => {
    if (!subscription) return

    // En iOS Safari, verificar conexión WebSocket
    if (platform === 'ios-safari' && false) {
      if (!isConnected) {
        console.log('WebSocket desconectado, esperando reconexión automática...')
      }
      return
    }
    ...
    try {
      ...
      if (!isValid) {
        console.log('Suscripción expirada, renovando automáticamente...')
        await subscribeToNotifications(permission === 'granted')
      }
    } catch (error) {
      console.error('Error verificando suscripción:', error)
    }
  }, [subscription, validateSubscription, subscribeToNotifications, platform, isConnected])

  const unsubscribeFromNotifications = useCallback(async (serverUrl) => {
    if (!subscription) return
  ...
  const sendTestNotification = useCallback(async (serverUrl) => {
    if (!subscription) {
    ...
    try {
      const response = await fetch(`${serverUrl}/notifications/test`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        ...
        body: JSON.stringify({
          ...
          platform: platform,
          type: platform === 'ios-safari' ? 'websocket' : 'push'
        })
      })
  ...
  const testServiceWorkerConnection = useCallback(async () => {
    if ('serviceWorker' in navigator) {
      try {
        ...
        if (navigator.serviceWorker.controller) {
          console.log('✅ Controller activo:', navigator.serviceWorker.controller)
          console.log('Estado del controller:', navigator.serviceWorker.controller.state)
  ...
  useEffect(() => {
    const autoSubscribe = async () => {
      ...
      if (
        ...
        console.log('🔄 Suscripción automática detectada - pushEndpoint existente:', pushEndpoint)
        ...
        try {
          ...
          const result = await subscribeToNotifications(permission === 'granted')
          if (result) {
  ...
  }, [pushEndpoint, permission, isSupported, token, subscription, subscribeToNotifications])
